...
public:
	static set<char> primero ( char a, Gramatica &g ) {
		...
		if ( g.tiene_terminal(a) || a == '3') { // 'a' Es un terminal
			...
			//for ( auto x: producciones ) {
			//	cout << x.obtener_cuerpo() << " " ;
			//}
...
public:
	...
	void agregar_entrada( char fila, char columna, int numero_produccion ) {
		printf( "Agregando: [%c][%c] = %d\n", fila, columna, numero_produccion);  
...
public:
	void generar_tablaLL1 ( string ruta_de_gramatica, string ruta_escritura ) {
		...
		cout << "inicial: " << g.obtener_simbolo_inicial() << endl;
		...
		for ( auto produccion_numerada : producciones_numeradas ) {
			...
			cout << "Producción: " << produccion_numerada.second << ":" << produccion_numerada.first.obtener_cabeza() << "->" << produccion_numerada.first.obtener_cuerpo() << endl;
			set<char> primero_alfa = CalculadorPrimero :: primero( produccion_numerada.first.obtener_cuerpo(), g );
			cout << "Primero: ";
			for ( auto x : primero_alfa ) {
				cout << x << " ";
			}
			cout << endl;
		...
		escritor.escribir_tabla_en("Tabla.txt", tabla);
	}
};
...
int main () {
	//Gramatica g;
	//LectorDeGramatica lector("Gramatica.txt");
	//g = lector.leer_gramatica_de("Gramatica.txt");
	////g.imprimir();
	//set<char> t;
	//// set<char> t = CalculadorPrimero :: primero( "BC", g );
	//t = CalculadorSiguiente :: siguiente( 'B', g );
	//for ( auto c : t )
	//	cout << c << " " << endl;

	GeneradorDeTablaLL1 generador;
	generador.generar_tablaLL1("Gramatica.txt", "Tabla.txt");
	return 0;
}